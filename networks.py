...
class ActorNetwork(eqx.Module):
    ...
    def __init__(self, key, in_shape, hidden_features: List[int], orthogonal_init, num_actions):
...
class ValueNetwork(eqx.Module):
    ...
    def __init__(self, key, in_shape, hidden_layers: List[int], orthogonal_init, **kwargs):
...
class Q_Network(eqx.Module):
    ...
    def __init__(self, key, in_shape, hidden_layers: List[int], orthogonal_init, num_actions):
        keys = jax.random.split(key, len(hidden_layers))
        self.layers = [
            CustomLinear(orthogonal_init, np.sqrt(2), in_shape, hidden_layers[0], key=keys[0])
        ] + [
            ...
            CustomLinear(orthogonal_init, 0.01, hidden_layers[-1], num_actions, key=keys[-1])
        ]
